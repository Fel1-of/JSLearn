class Validator {
  constructor({selector, method, pattern = {}}) {
    this.form = document.querySelector(selector);
    this.pattern = pattern;
    this.method = method;
    this.elementsForm = [...this.form.elements].filter(item => {return item.tagName.toLowerCase() !== 'button' && item.type !== 'button'});
    this.error = new Set();
  }
  ...
  isValid(elem) {
    ...
    if (this.method) {
      ...
      console.warn('Необходимо передать идентификаторы полей ввода и методы проверки этих полей');
    }
